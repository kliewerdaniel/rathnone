...
# --- News Evidence Surface (ADR 48) — opt-in, fail-closed off by default ---
RATHNONE_NEWS_ENABLED=1 RATHNONE_NEWS_API_KEY=newskey \
RATHNONE_EVIDENCE_KEY_PEM=path/to/evidence_key.pem \
RATHNONE_EVIDENCE_OP_KEY_PEM=path/to/evidence_op_key.pem \
  python -m uvicorn src.news.service:app --port 8792
```
...
examples/             runnable PoCs: agent_harness.py, live_harness.py, harness_loop.py, harness_memory_demo.py, harness_memory_loop.py, news_surface_demo.py
tests/                382 tests (gateway, security, query engine, live transport, poison guard, harness consumer, skill distillation, no-data-loss audit, local MCP, harness memory, news evidence surface)
