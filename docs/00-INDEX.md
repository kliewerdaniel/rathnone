...
| `47-LOCAL-HARNESS-MEMORY.md` | ADR 47 RATIFIED+IMPLEMENTED (Phase 2): persona MoE GraphRAG over **self-hosted** local Neo4j + Ollama `nomic-embed-text` — `src/harness/memory.py` with INVESTIGATED edges, verbatim `LiteralFact`, combined `is_drift` quarantine, in-memory fallback |
