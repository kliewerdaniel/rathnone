...
**Status:** Implemented & verified. Finance gateway (ADR 17–24, 26) + knowledge-query engine (ADR 27–40) shipped and green, plus the agent-harness authority binding (ADR 41 — harness as a `decide()` consumer with a fail-closed gate + operator `/safety/halt`), the harness capability split (ADR 42 — `explore`/`execute` so `decide()` HUMAN-gates only consequential actions), and the signed-execute gate (ADR 43 — `execute` is hard-blocked until a cryptographically-bound `OperatorCommand` arrives, no local self-approval). Full suite: **368 pytest passing** (incl. ADR 36 rotation-aware witness audit, ADR 37 cross-surface root-of-trust, ADR 38 live cross-surface attestation consumer, ADR 39 eval suite — 7,056-pt decision sweep + 5,000-vector blind adversary harness, 0 false accepts/auths, ADR 40 knowledge-layer source-corroboration guard — distinct-origin quorum closes the "one-feed/one-principal" poison loophole, ADR 41 harness gate verified over real TCP, ADR 42 explore/apply split, ADR 43 signed-execute over real TCP with replay/body-mismatch rejection). The knowledge substrate is proven both in-process (`TestClient`) and over a real TCP socket (`uvicorn` + `httpx`, ADR 33). Live track, hygiene gate, operator downgrade, evidence scope, the poison guard, and the harness gate are opt-in, fail-closed.

**Phase 2 (local-mirror, `phase2-local-mirror` branch):** four ADRs ratify folding useful local-first skill patterns into Rathnone without touching the frozen spine — ADR 44 skill distillation (`scripts/distill_skill.py` + `skills/ratify-an-adr/SKILL.md`, provenance-checked regen), ADR 45 no-data-loss audit (`src/query/audit.py` + `scripts/evidence_witness_verify.py --graph`, anchor-first cardinality assertion), ADR 46 local MCP surface (`src/mcp_local/server.py` — hand-rolled JSON-RPC over stdio, Read requires a signed `QueryScope`, Read-write requires a signed `OperatorCommand`, localhost-only Neo4j mirror guard), and ADR 47 local harness memory (persona MoE GraphRAG over **self-hosted** Neo4j `bolt://127.0.0.1:7687` + local Ollama `nomic-embed-text` — INVESTIGATED edges, verbatim `LiteralFact`, combined `is_drift` quarantine rule, in-memory fallback; consumed by `examples/harness_memory_loop.py` (a living harness loop that records each investigation and fails closed on drift — an off-domain action is quarantined). Full suite: **368 pytest passing**. Bonus surface **ADR 48 News Evidence Surface** (objective news corroboration — `src/news/` reuses the existing `src/query` engine: RSS/Atom/JSON feeds -> the same `KnowledgeGraph`, ADR 40 distinct-origin quorum + ADR 35 witness log reused verbatim; the verdict states "N distinct outlets agree / 1 source un-corroborated / outlets diverge", never world-truth) lives on its own `news-evidence-surface` branch.
...
| `44-SKILL-DISTILLATION.md` | ADR 44 RATIFIED+IMPLEMENTED (Phase 2): mirror `nams-skill-distillation` — `scripts/distill_skill.py` regenerates `skills/ratify-an-adr/SKILL.md` with provenance-checked content hashes of cited ADRs |
| `45-NO-DATA-LOSS-AUDIT.md` | ADR 45 RATIFIED+IMPLEMENTED (Phase 2): mirror `cypher-no-data-loss` — `src/query/audit.py` anchor-first entity enumeration + cardinality assertion; `scripts/evidence_witness_verify.py --graph` cross-checks the live witness log (no silent row-drop) |
| `46-LOCAL-MCP-SURFACE.md` | ADR 46 RATIFIED+IMPLEMENTED (Phase 2): mirror `mcp-for-aura` safety model on local stdio — `src/mcp_local/server.py` hand-rolled JSON-RPC, Read requires signed `QueryScope`, Read-write requires signed `OperatorCommand`, localhost-only Neo4j mirror guard |
| `47-LOCAL-HARNESS-MEMORY.md` | ADR 47 RATIFIED+IMPLEMENTED (Phase 2): persona MoE GraphRAG over **self-hosted** local Neo4j + Ollama `nomic-embed-text` — `src/harness/memory.py` with INVESTIGATED edges, verbatim `LiteralFact`, combined `is_drift` quarantine, in-memory fallback; `examples/harness_memory_loop.py` is the living consumer (fails closed on drift) |
| `48-NEWS-EVIDENCE-SURFACE.md` | ADR 48 RATIFIED+IMPLEMENTED: objective News Evidence Surface — `src/news/` reuses the existing `src/query/` engine UNCHANGED (feeds -> same `KnowledgeGraph`, ADR 40 distinct-origin quorum + ADR 35 witness log reused verbatim); `NewsAgent` wraps `KnowledgeAgent`, `NewsVerdict` states "N distinct outlets agree / 1 source un-corroborated / outlets diverge" (never world-truth); fail-closed off by default (`RATHNONE_NEWS_ENABLED=0`); on its own `news-evidence-surface` branch |
