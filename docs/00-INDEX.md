...
**Status:** Implemented & verified. Finance gateway (ADR 17–24, 26) + knowledge-query engine (ADR 27–40) shipped and green. Full suite: **294 pytest passing** (incl. ADR 36 rotation-aware witness audit, ADR 37 cross-surface root-of-trust, ADR 38 live cross-surface attestation consumer, ADR 39 eval suite — 7,056-pt decision sweep + 5,000-vector blind adversary harness, 0 false accepts/auths, ADR 40 knowledge-layer source-corroboration guard — distinct-origin quorum closes the "one-feed/one-principal" poison loophole). The knowledge substrate is proven both in-process (`TestClient`) and over a real TCP socket (`uvicorn` + `httpx`, ADR 33). Live track, hygiene gate, operator downgrade, evidence scope, and the poison guard are opt-in, fail-closed.
