...
**Status:** Implemented & verified. Finance gateway (ADR 17–24, 26) + knowledge-query engine (ADR 27–39) shipped and green. Full suite: **282 pytest passing** (incl. ADR 36 rotation-aware witness audit, ADR 37 cross-surface root-of-trust, ADR 38 live cross-surface attestation consumer, ADR 39 eval suite — 7,056-point decision sweep + 5,000-vector blind adversary harness, 0 false accepts/auths). The knowledge substrate is proven both in-process (`TestClient`) and over a real TCP socket (`uvicorn` + `httpx`, ADR 33). Live track, hygiene gate, operator downgrade, and evidence scope are opt-in, fail-closed.
