...
3. **`authority`/`confidence` are advisory, never corroboration.** A high score
   from a single origin buys nothing; `scores_treated_as_advisory` is surfaced in
   the report so a downstream agent can *see* the scores without trusting them as
   a quorum. We do NOT block on a score being absent (every real SKC artifact
   carries scores — blocking would false-positive the corpus we protect).
...
5. **Service wiring (ADR 40 seam).** `service.create_app()` builds a
   `PurificationLayer` from `RATHNONE_PURIFY_ENABLED` / `RATHNONE_PURIFY_QUORUM`
   (env, call-time, per-instance). Every `/query/*` response carries
   `out["poison"]` = the verdict dict. Disabled by default => pure pass-through,
   so the existing 282-suite is unaffected.
6. **Consumer narrowing (`KnowledgeAgent.accept`).** The agent is given a
...
- **Structural, not semantic.** It catches sybil provenance, single-origin
  reliance, and total-graph capture. It does **NOT** catch *semantic* poison: text
  that is internally consistent and sourced from genuinely distinct reputable
  origins but false in the world (the model-reasoning-from-wrong-fact case). We
  claim *"the evidence set has enough distinct origins to not be a single-
  principal fabrication,"* not *"the claims are true."* That limit is stated in the
  report and here.
...
- Full suite: **294 pytest passing** (prior 282 + 12). The layer is off in every
  existing test (env unset), so no regression.
