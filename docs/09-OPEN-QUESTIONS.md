...
- **Knowledge poisoning (the source's open problem).** A perfectly governed system can still do the wrong thing if its *knowledge* is wrong: model reasons correctly from wrong knowledge, policy authorizes, execution succeeds, verification confirms the expected (wrong) state — every invariant passes, outcome undesirable. Rathnone inherits this. *v1 scope: out of scope* for the authority product. **Mitigation shipped (ADR 40):** a narrowing-only, fail-closed, opt-in `PurificationLayer` at the knowledge plane applies the ADR 24 distinct-origin-quorum discipline to retained evidence — it defeats sybil provenance (one principal, many domains) and one-feed inflation by collapsing to registrable origins and stamping a retained set `CLEAN`/`POISONED`. **Semantic slice (bounded):** ADR 40 *also* catches (a) internal contradiction — if a retained set holds BOTH sides of a corpus-flagged `contradictions[]` opposition (an agent holding both would believe opposites), and (b) unearned confidence — a high-score entity on provenance that fails the distinct-origin quorum. Both are detected from the *corpus's own structured signals*, not by an LLM. **Honest limit:** ADR 40 still does NOT verify *world-truth* — text that is internally consistent, well-sourced, unflagged, but false in the world passes. Closing that gap needs an external fact-oracle (network + model), which the frozen local-first spine forbids. That remains an open research track, not a hidden gap.
